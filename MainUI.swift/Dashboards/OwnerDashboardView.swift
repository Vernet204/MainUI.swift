...
struct OwnerDashboardView: View {
    ...
    // MARK: - Alerts
    @State private var alertItems: [OwnerAlert] = []

    // MARK: - Recent Activity
    @State private var recentLoads: [RecentLoadItem] = []
    @State private var recentReports: [ReportItem] = []
        ...
        loadListener = Firestore.firestore()
            ...
            .addSnapshotListener { snapshot, _ in
                ...
                for doc in docs {
                    ...
                    // ✅ Recent loads — last 5 by createdAt
                ...
                DispatchQueue.main.async {
                    ...
                    // Merge alerts (keep vehicle alerts already set)
                    let vehicleAlerts = alertItems.filter { $0.type == .vehicleInMaintenance }
                    let reportAlerts = alertItems.filter { $0.type == .openReport }
                    alertItems = alerts + vehicleAlerts + reportAlerts
                    isLoading = false
                }
            }
    }
            ...
            .addSnapshotListener { snapshot, _ in
                ...
                for doc in docs {
                    ...
                    if status == "In Maintenance" {
                        maintenance += 1
                        vehicleAlerts.append(OwnerAlert(
                            id: doc.documentID,
                ...
                DispatchQueue.main.async {
                    ...
                    let loadAlerts = alertItems.filter { $0.type == .declinedLoad }
                    let reportAlerts = alertItems.filter { $0.type == .openReport }
                    alertItems = loadAlerts + vehicleAlerts + reportAlerts
                }
            }
    }
        ...
        reportListener = Firestore.firestore()
            ...
            .addSnapshotListener { snapshot, _ in
                ...
                for doc in docs {
                    ...
                    open += 1
                    reportAlerts.append(OwnerAlert(
                        id: doc.documentID,
                        type: .openReport,
