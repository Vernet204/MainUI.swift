                        ...
                        VStack(alignment: .leading, spacing: 10) {
                            ...
                            HStack(alignment: .top) {
                                ...
                                Text("Location: \(locationString)")
                                    .font(.subheadline)
                                    .foregroundColor(locationString == "Fetching location..." ? .gray : .primary)
                            }
                        }
                    }
                    ...
                    Button(action: submitReport) {
                        if isSubmitting {
                            ProgressView()
                                ...
                                .background(Color.red.opacity(0.6)).cornerRadius(12)
                        } else {
                            Text("SUBMIT REPORT")
        ...
        for item in items {
            item.loadTransferable(type: Data.self) { result in
                if case .success(let data?) = result, let image = UIImage(data: data) {
                    DispatchQueue.main.async { uiImages.append(image) }
                }
            }
        }
    }
