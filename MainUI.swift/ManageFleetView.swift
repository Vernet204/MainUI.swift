...
struct ManageFleetView: View {
    ...
    func deleteVehicles(at indexSet: IndexSet) {
        indexSet.map { filteredVehicles[$0] }.forEach { vehicle in
            Firestore.firestore().collection("vehicles").document(vehicle.id).delete()
        }
    }
 
    func deleteEmployees(at indexSet: IndexSet) {
        indexSet.map { filteredEmployees[$0] }.forEach { employee in
            Firestore.firestore().collection("users").document(employee.id).delete()
        }
    }
}
        ...
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                VStack(alignment: .leading, spacing: 2) {
                    Text("Unit \(vehicle.unitNumber)").font(.headline).foregroundColor(.primary)
                    Text(vehicle.plate).font(.caption).foregroundColor(.secondary)
                }
                Spacer()
                Text(vehicle.status)
            ...
            if let date = vehicle.lastInspectionDate {
                HStack(spacing: 4) {
                    ...
                    if !vehicle.lastInspectedBy.isEmpty {
                        Text("by \(vehicle.lastInspectedBy)").font(.caption2)
                    }
                }
...
struct EmployeeRowCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                Image(
                    systemName: employee.role.lowercased() == "driver"
                ...
                VStack(alignment: .leading, spacing: 2) {
                    Text(employee.name).font(.headline).foregroundColor(.primary)
                    Text(employee.email).font(.caption).foregroundColor(.secondary)
                }
...
struct FleetVehicleDetailView: View {
        ...
        NavigationStack {
            List {
                Section("Vehicle Info") {
                    if isEditMode {
                        HStack {
                            Text("Unit Number").foregroundColor(.secondary).frame(width: 110, alignment: .leading)
                            TextField("Unit Number", text: $unitNumber)
                        }
                        HStack {
                            Text("License Plate").foregroundColor(.secondary).frame(width: 110, alignment: .leading)
                ...
                Section("Driver Assignment") {
                    if isEditMode {
                        ...
                        if assignedDriver != nil {
                            Button("Unassign Driver") { showUnassignConfirm = true }
                                .foregroundColor(.red)
                        }
                    } else {
                        if let driver = assignedDriver {
                            Label(driver.name, systemImage: "person.fill").foregroundColor(.blue)
                        } else {
                ...
                Section("Inspection") {
                    if !vehicle.inspectionStatus.isEmpty {
                        HStack {
                            Text("Status").foregroundColor(.secondary)
                            Spacer()
                            Text(vehicle.inspectionStatus).fontWeight(.semibold)
                                .foregroundColor(inspectionStatusColor(vehicle.inspectionStatus))
                        }
                    }
                    if let date = vehicle.lastInspectionDate {
            ...
            .navigationBarTitleDisplayMode(.large)
            .onAppear { loadFields(); fetchDrivers() }
            .toolbar {
                ToolbarItem(placement: .topBarLeading) {
                    Button(isEditMode ? "Cancel" : "Done") {
                        if isEditMode { loadFields(); isEditMode = false } else { dismiss() }
                    }
                }
                ToolbarItem(placement: .topBarTrailing) {
                    Button(isEditMode ? "Save" : "Edit") {
                        if isEditMode { saveChanges() } else { isEditMode = true }
                    }
                    .fontWeight(isEditMode ? .bold : .regular)
                }
            }
            .confirmationDialog("Unassign Driver?",
                                isPresented: $showUnassignConfirm,
                                titleVisibility: .visible) {
                Button("Yes, Unassign", role: .destructive) { assignedDriver = nil }
    ...
    func vehicleStatusColor(_ status: String) -> Color {
        switch status {
        case "Active": return .green; case "In Maintenance": return .orange
        default: return .gray
        }
    }
    ...
    func inspectionStatusColor(_ status: String) -> Color {
        switch status {
        case "Passed", "Cleared": return .green
        case "Failed", "Accident Reported": return .red
        case "Needs Repair": return .orange
        default: return .gray
        }
    }
        ...
        NavigationStack {
            List {
                Section("Employee Info") {
                    if isEditMode {
                        HStack {
                            Text("Name").foregroundColor(.secondary).frame(width: 80, alignment: .leading)
                            TextField("Full Name", text: $name).textInputAutocapitalization(.words)
                        }
                        HStack {
                            Text("Phone").foregroundColor(.secondary).frame(width: 80, alignment: .leading)
                            TextField("Phone Number", text: $phone).keyboardType(.phonePad)
                        }
                    } else {
                        DetailRow(label: "Name", value: name)
                        DetailRow(label: "Email", value: email)
                        if !phone.isEmpty { DetailRow(label: "Phone", value: phone) }
                    }
                }
                ...
                Section("Role") {
                    if isEditMode {
                        ...
                        HStack {
                            Image(systemName: role.lowercased() == "driver" ? "steeringwheel" : "headphones")
                        ...
                        if isEditMode {
                            Picker("Assign Vehicle", selection: $selectedVehicle) {
                                ...
                                ForEach(availableVehicles) { v in
                                    Text("Unit \(v.unitNumber) — \(v.plate)").tag(Optional(v))
                                }
                            }.pickerStyle(.menu)
                        } else {
                            if let v = selectedVehicle {
                                Label("Unit \(v.unitNumber) — \(v.plate)", systemImage: "truck.box.fill")
            ...
            .navigationBarTitleDisplayMode(.large)
            .onAppear { loadFields(); fetchVehicles() }
            .toolbar {
                ToolbarItem(placement: .topBarLeading) {
                    Button(isEditMode ? "Cancel" : "Done") {
                        if isEditMode { loadFields(); isEditMode = false } else { dismiss() }
